...
class PanoramaEditor extends PanoramaBase {
    ...
    async loadDefaultPanorama(config) {
        ...
        try {
            ...
            // Используем унифицированный formatHotSpots
            scene.hotSpots = this.formatHotSpots(hotspots, this.basePath, resolvedImage);
    ...
    async finalizeScene(imageUrl, isExternal, hotspotsData, cameraDirection, hotSpot, hotSpotsValue, config, relativePath) {
        ...
        // ИСПОЛЬЗУЕМ УНИФИЦИРОВАННЫЙ formatHotSpots
        var formattedHotspots = this.formatHotSpots(uniqueHotspots, this.basePath, imageUrl);
     ...
     * ИСПОЛЬЗУЕТ navigator.clipboard с fallback
     */
    async copyPathWithSeparation() {
        if (this.isInIframe) return;
        ...
        // Используем navigator.clipboard с fallback
        try {
            await navigator.clipboard.writeText(copyText);
            console.log('Copied with separation using clipboard API:', { path: result.path, photo: result.photo });
        } catch (err) {
    ...
    loadPreview() {
        ...
        // Используем унифицированный buildFullUrl
        var fullUrl = this.buildFullUrl(path, photo);
    ...
    updateSceneWithHotspots() {
        ...
        this.currentHotspots.forEach(function(hs, index) {
            // Используем унифицированный buildFullUrl
            var fullUrl = self.buildFullUrl(hs.path || self.basePath || '', hs.panorama_url);
    ...
    editHotspot(index) {
        ...
        var fullUrl = this.buildFullUrl(hotspot.path || this.basePath || '', hotspot.panorama_url);
        document.getElementById('fullUrlDisplay').value = fullUrl;
    ...
    /**
     * УСТАРЕВШИЙ МЕТОД: Копирование через execCommand (используется как fallback)
     */
    ...
    showCustomContextMenu(x, y, data) {
        ...
        if (data.type === 'hotspot') {
            var safeData = safeJsonStringify(data.data);
            var safeCoords = safeJsonStringify(data.coords);
            menu.innerHTML =
                '<div class="menu-item" data-action="edit" data-data="' + safeData + '" data-coords="' + safeCoords + '" style="padding:5px 10px;cursor:pointer;">✏️ Edit HotSpot</div>' +
                '<div class="menu-item" data-action="delete" data-data="' + safeData + '" data-coords="' + safeCoords + '" style="padding:5px 10px;cursor:pointer;">🗑️ Delete HotSpot</div>' +
                '<hr style="margin:5px 0;">' +
                '<div class="menu-item" data-action="copy" data-data="' + safeCoords + '" style="padding:5px 10px;cursor:pointer;">📋 Copy Coordinates</div>';
        } else {
            var safeData = safeJsonStringify(data.data);
            menu.innerHTML =
                '<div class="menu-item" data-action="add" data-data="' + safeData + '" style="padding:5px 10px;cursor:pointer;">➕ Add HotSpot Here</div>' +
                '<div class="menu-item" data-action="copy" data-data="' + safeData + '" style="padding:5px 10px;cursor:pointer;">📋 Copy Coordinates</div>' +
                '<hr style="margin:5px 0;">' +
                '<div class="menu-item" data-action="center" data-data="' + safeData + '" style="padding:5px 10px;cursor:pointer;">🎯 Center View Here</div>';
        }
        ...
        menu.addEventListener('click', function(e) {
            ...
            var dataAttr = target.dataset.data;
            var coordsAttr = target.dataset.coords;

            if (!dataAttr) return;

            try {
                var parsedData = JSON.parse(dataAttr);
                var parsedCoords = coordsAttr ? JSON.parse(coordsAttr) : null;

                switch(action) {
                    case 'add':
                        window.panoramaEditor.addHotSpotAt(parsedData);
                        break;
                    case 'edit':
                        window.panoramaEditor.handleHotSpotAction('edit', parsedData);
                        break;
                    case 'delete':
                        window.panoramaEditor.handleHotSpotAction('delete', parsedData);
                        break;
                    case 'copy':
                        window.panoramaEditor.copyCoords(parsedData);
                        break;
                    case 'center':
                        window.panoramaEditor.centerView(parsedData);
                        break;
                    default:
    ...
    handleHotSpotAction(action, hotSpot) {
        ...
        // Если это редактирование - нужно найти индекс хотспота и открыть модалку
        if (action === 'edit' && hotSpot && hotSpot.id) {
            var index = this.currentHotspots.findIndex(function(h) {
                return h.id === hotSpot.id;
            });
            if (index !== -1) {
                this.editHotspot(index);
                return;
            }
        }

        // Если это удаление
        if (action === 'delete' && hotSpot && hotSpot.id) {
            var index = this.currentHotspots.findIndex(function(h) {
                return h.id === hotSpot.id;
            });
            if (index !== -1) {
        ...
        alert(action + ' HotSpot: ' + (hotSpot.text || hotSpot.type || 'Без имени'));
    }
    ...
    /**
     * Добавляет точку перехода по указанным координатам (вызывается из контекстного меню)
     * @param {Object} coords - Объект с полями pitch и yaw
     */
    addHotSpotAt(coords) {
        if (this.isInIframe) return;
        console.log('Add HotSpot at:', coords);
        // Открываем модальное окно с переданными координатами (аналогично двойному клику)
