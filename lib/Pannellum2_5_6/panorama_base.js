...
class PanoramaBase {
    ...
    /**
     * Получает корневой путь из текущего URL (без имени страницы)
    ...
    resolveImagePath(imagePath, overridePath = null) {
        ...
        // Если basePath не указан, используем корень из текущего URL
        if (!basePath) {
            var currentUrl = window.location.href;
            var lastSlashIndex = currentUrl.lastIndexOf('/');
            basePath = currentUrl.substring(0, lastSlashIndex);
            console.log('Using root path from current URL:', basePath);
        }
