...
class PanoramaBase {
    ...
    /**
     * УНИФИЦИРОВАННЫЙ МЕТОД: Собирает полный URL из базового пути и относительного
     * @param {string} basePath - Базовый путь (может быть URL или относительный)
     * @param {string} relativePath - Относительный путь к файлу
     * @returns {string} Полный URL
     */
    buildFullUrl(basePath, relativePath) {
        if (!relativePath) return '';

        // Если relativePath уже абсолютный - возвращаем как есть
        if (this.isExternalUrl(relativePath)) {
            return relativePath;
        }

        // Если basePath не указан, используем корневой путь из URL
        if (!basePath) {
            basePath = this.basePath || this.getRootPathFromUrl();
        }

        // Очищаем пути от лишних слешей
        var cleanBase = String(basePath).replace(/\/+$/, '');
        var cleanRelative = String(relativePath).replace(/^\/+/, '');

        // Если cleanBase пуст, используем корневой путь
        if (!cleanBase) {
            cleanBase = this.getRootPathFromUrl();
        }

        // Формируем результат
        var result = cleanBase + '/' + cleanRelative;
        console.log('buildFullUrl:', { basePath, relativePath, result });
        return result;
    }
    ...
    /**
     * УНИФИЦИРОВАННЫЙ МЕТОД: Форматирует хотспоты для Pannellum
     * @param {Array} hotspots - Массив объектов хотспотов
     * @param {string} basePath - Базовый путь (опционально)
     * @param {string} imageUrl - URL текущей панорамы (для определения базового пути)
     * @returns {Array} Отформатированные хотспоты
     */
    formatHotSpots(hotspots, basePath = null, imageUrl = null) {
        if (!Array.isArray(hotspots)) return [];

        var self = this;
        // Определяем базовый путь
        var effectiveBasePath = basePath || this.basePath || '';

        // Если basePath не указан, но есть imageUrl - берем путь из него
        if (!effectiveBasePath && imageUrl) {
            var parts = imageUrl.split('/');
            parts.pop();
            effectiveBasePath = parts.join('/');
        }

        return hotspots.map(function(hotspot) {
            var fullUrl = hotspot.panorama_url || '';

            // Если panorama_url относительный - строим полный URL через buildFullUrl
            if (fullUrl && !self.isExternalUrl(fullUrl)) {
                // Используем путь из hotspot, если есть
                var hsPath = hotspot.path || effectiveBasePath;
                fullUrl = self.buildFullUrl(hsPath, fullUrl);
            }
            ...
            return {
                ...
                path: hotspot.path || effectiveBasePath
            };
        });
    }
    ...
    /**
     * Получает JSON данные через XHR (синхронно) - УСТАРЕЛО, использовать loadHotSpotsFromJson
